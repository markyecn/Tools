...
static VENCalculatorInputView *instance;

@implementation VENCalculatorInputView

+(instancetype)sharedInputView{
    if (!instance) {
        instance = [VENCalculatorInputView new];
    }
    return instance;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self setOperationButtonBackgroundColor:[UIColor colorWithWhite:0.98828 alpha:1]];
...
- (IBAction)hideBtnClicked:(id)sender {
    [[UIDevice currentDevice] playInputClick];
    if ([self.delegate respondsToSelector:@selector(calculatorInputViewDidTapHideBtn:)]) {
        [self.delegate calculatorInputViewDidTapHideBtn:self];
    }
}

- (IBAction)confirmBtnClicked:(id)sender {
    [[UIDevice currentDevice] playInputClick];
    if ([self.delegate respondsToSelector:@selector(calculatorInputViewDidTapConfirmBtn:)]) {
        [self.delegate calculatorInputViewDidTapConfirmBtn:self];
    }
}
